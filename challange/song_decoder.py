...
# Output
# Return the words of the initial song that Polycarpus used to make a dubsteb remix. Separate the words with
# a space.
...
def song_decoder(song) :
    ...
    song = song.replace("WUB"," ").split(" ")   # song değişkenine atama yapmadan bu expression ı return
                                                # ettiğimizde tüm boşlukları tek bir boşluk haline getiriyor.
    i = 0
    while i < len(song) :               # while döngüsünde i=i-1 kodu döngü üzerinde remove işlemini nötralize
        if song[i] == "":               # ediyor.
           song.remove(song[i])
           i = i -1
        i +=1

    song = " ".join(song).strip() 

    return song

print(song_decoder("WUBWUBIWUBWUBAMWUBWUBWUBWUBWUBXWUB"))